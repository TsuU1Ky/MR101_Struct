...
class TestTP2(unittest.TestCase):
    ...
    def test_triBulles(self):
        for i in range(10_000):
            ...
            triBulles(l1)
            l2.sort()
            self.assertEqual(l2, l1, f"La liste {l1} n'est pas triée, on devrait avoir {l2}")
    ...
    def test_getIndexMin(self):
        for i in range(1000):
            ...
            self.assertEqual(min(lst), lst[idx],
                             f"l'indice {idx} dans la liste {lst} ne correspond pas à la valeur minimale {min(lst)}")
    ...
    f"l'indice {idx} dans la liste {lst} ne correspond pas à la valeur minimale {min(lst[dep:])} ou la position {idx} est inférieur au début de la recherche {dep}")
    ...
    def test_deplacerCase(self):
        for i in range(1000):
            ...
            self.assertEqual(l3, l2, f"Le déplacement de la case d'indice {pos} de la liste {l1} aurait dû donner {l3} au lieu de {l2}")
